...
def download_data(browser,date_button):
    ...
    #time.sleep(5)
    #点击镍的日交易排名标签
    wait.until(lambda driver:driver.find_element_by_id('li_hc'))
    try:
        browser.find_element_by_id('li_hc').click()
    except:
        pass
...
if __name__=='__main__':
    ...
    all_data=all_data[all_data['category']=='hc']
    all_data=all_data.drop_duplicates()
    print all_data
    #all_data.to_csv("all_data_Cu.csv",encoding='gbk',index=False)
    all_data.to_sql("hc_volume_data",engine,index=False,if_exists='append')
    browser.close()
    print 'done!'