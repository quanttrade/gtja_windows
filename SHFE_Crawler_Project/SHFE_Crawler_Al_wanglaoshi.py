...
if __name__=='__main__':
    ...
    file_path='C:\\Users\\len\\Downloads\\data.csv'
    category='al'
    ...
    try:
        exist_data=pd.read_csv('existing_%s.csv'%category)
        exist_data['update_date']=pd.to_datetime(exist_data['update_date'])
        max_date=pd.to_datetime(np.max(exist_data['update_date']))
    except:
    ...
    all_data=exist_data.append(all_data)
    all_data.to_csv("existing_%s.csv",encoding='gbk',index=False,if_exists='replace')
