...
#         index_data['trend']=index_data['trend'].shift(1)
    ...
    def stop_loss(self,trade_signal,close_high,close_low,return_rate,threshold=-0.04):
        return stop_loss(trade_signal,close_high,close_low,return_rate,threshold=-0.04)
    ...
    def stop_loss_trades(self):
        ...
        for i,j in df.groupby(['company_name','trade_num']):
            ...
            if np.sum(np.isnan(trade_signal))!=len(trade_signal) and np.isnan(trade_signal[0])==False:
    ...
    def plot_fig(self):
        ...
        position=position.groupby(['update_date']).position_signal.sum()
